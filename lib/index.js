...
var JoinAndLeaveLog = '310791614924587032'
var MessageDeleteLog = '310791925617917962'
var MessageEditLog = '310792396814155776'
var NameChangeLog = '310794181905743872'
...
client.on('guildMemberAdd', member => {
    if (JoinAndLeaveLog !== null) {
        if (member.user.bot === false) {
            client.channels.get(JoinAndLeaveLog).sendMessage(":information_source: **" + member.user.username + "** has joined **" + member.guild.defaultChannel.guild.name + " [USER ID: " + member.user.id + "]**");
//            if ((member.user.username.toLowerCase().match('faggot'))) {
//                client.channels.get("228252059181383683").sendMessage("@everyone \n" +
//                                                                    ":warning: **SUSPICIOUS USERNAME DETECTED** \n" +
//                                                                    "          **Username:** " + member.user.username +
//                                                                    "\n          **Discord:** "+ member.guild.defaultChannel.guild.name);
//                setTimeout(function(){ member.kick() }, 100);
//            }
//            if ((member.user.username.toLowerCase().match('nigger'))) {
//                client.channels.get("228252059181383683").sendMessage("@everyone \n" +
//                                                                    ":warning: **SUSPICIOUS USERNAME DETECTED** \n" +
//                                                                    "          **Username:** " + member.user.username + 
//                                                                    "\n          **Discord:** "+ member.guild.defaultChannel.guild.name);
//                setTimeout(function(){ member.kick() }, 100);
//            }
//            if ((member.user.username.toLowerCase().match('rape'))) {
//                client.channels.get("228252059181383683").sendMessage("@everyone \n" +
//                                                                    ":warning: **SUSPICIOUS USERNAME DETECTED** \n" +
//                                                                    "          **Username:** " + member.user.username + 
//                                                                    "\n          **Discord:** "+ member.guild.defaultChannel.guild.name);
//                setTimeout(function(){ member.kick() }, 100);
//            }
//            if ((member.user.username.toLowerCase().match('kevin'))) {
//                client.channels.get("228252059181383683").sendMessage("@everyone \n" +
//                                                                    ":warning: **SUSPICIOUS USERNAME DETECTED** \n" +
//                                                                    "          **Username:** " + member.user.username + 
//                                                                    "\n          **Discord:** "+ member.guild.defaultChannel.guild.name);
//                setTimeout(function(){ member.kick() }, 100);
//            }
//            if ((member.user.username.toLowerCase().match('racist'))) {
//                client.channels.get("228252059181383683").sendMessage("@everyone \n" +
//                                                                    ":warning: **SUSPICIOUS USERNAME DETECTED** \n" +
//                                                                    "          **Username:** " + member.user.username + 
//                                                                    "\n          **Discord:** "+ member.guild.defaultChannel.guild.name);
//                setTimeout(function(){ member.kick() }, 100);
//            }
//            if ((member.user.username.toLowerCase().match('dick'))) {
//                client.channels.get("228252059181383683").sendMessage("@everyone \n" +
//                                                                    ":warning: **SUSPICIOUS USERNAME DETECTED** \n" +
//                                                                    "          **Username:** " + member.user.username + 
//                                                                    "\n          **Discord:** "+ member.guild.defaultChannel.guild.name);
...
//            }
            ...
            client.channels.get(JoinAndLeaveLog).sendMessage(":information_source: A new **Bot** named **" + member.user.username + "** has added into **" + member.guild.defaultChannel.guild.name + "**");
        }
    }
});
...
client.on('guildMemberRemove', member => {
    if (JoinAndLeaveLog !== null) {
        if (member.user.bot === false) {
            client.channels.get(JoinAndLeaveLog).sendMessage(":warning: **" + member.user.username + "** has left **" + member.guild.defaultChannel.guild.name + " [USER ID: " + member.user.id + "]**");
        }
        else {
            if (member.user.id !== client.user.id) {
                client.channels.get(JoinAndLeaveLog).sendMessage(":warning: A bot named **" + member.user.username + "** has been removed from **" + member.guild.defaultChannel.guild.name + "**");
            }
            else {
                client.channels.get(JoinAndLeaveLog).sendMessage("@everyone \n:warning: I have been kicked out from " + member.guild.defaultChannel.guild.name + "!**");
            }
        }
    }
});
...
client.on('messageDelete', message => {
    ...
    if (MessageDeleteLog !== null) {
        client.channels.get(MessageDeleteLog).sendMessage(":warning: **Message has been deleted from " + message.channel.guild.name + "\n**" +
...
client.on('messageUpdate', (oldMessage, newMessage) => {
    ...
    if (MessageEditLog !== null) {
        if (oldMessage.content !== newMessage.content) {
            client.channels.get(MessageEditLog).sendMessage(":warning: **Message has been edited in " + oldMessage.channel.guild.name + "\n**" +
                    "          **User:** " + oldMessage.member.user.username + " \n" +
...
client.on('guildMemberUpdate', (oldMember, newMember) => {
    ...
    if (NameChangeLog !== null) {
        if (oldMember.user.username !== newMember.user.username) {
            client.channels.get(NameChangeLog).sendMessage(":warning: **" + oldMember.user.username + " has changed their username to " + newMember.user.username + "**");
        }
        if (oldMember.nickname !== newMember.nickname) {
            client.channels.get(NameChangeLog).sendMessage(":warning: **" + oldMember.user.username + " has changed their nickname in " + oldMember.guild.defaultChannel.guild.name + "**\n" + 
...
client.on('ready', () => {
    ...
    client.channels.get('310787999862620171').sendMessage('<@' + OwnerId + '>\n:white_check_mark: **Electro-Atom Bot on duty!**')
...
client.on('message', message => {
    ...
    else if (message.content.toLowerCase().match('super')) {                                        //Mention "Super"
        DMOwner(":information_source: **You may have been mentioned from " + message.channel.guild.name + "\n**" +
                "          **User:** " + message.member.user.username + " \n" +
                "          **Channel:** " + message.channel + " \n" +
                "          **Message:** " + message.content)
    }
	else if (message.content.toLowerCase().match('azmil')) {
        DMOwner(":information_source: **You may have been mentioned from " + message.channel.guild.name + "\n**" +
                "          **User:** " + message.member.user.username + " \n" +
                "          **Channel:** " + message.channel + " \n" +
                "          **Message:** " + message.content)
    ...
    if (message.content === Prefix + 'dice') {                                        //Roll the Dice
        ...
        if (message.author.id === OwnerId) {
			console.log('Game to play:' + (message.content.slice(5 + Prefix.length)));
